...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		// Remember you can use Processing's graphics methods here
				...
				text("Size ~ Magnitude", 30, 220);
				
				fill(color(255, 255, 0));
				ellipse(15, 250, 10, 10);
				fill(0);
				text("--> Shallow", 30, 255);
				
				fill(color(0, 0, 255));
				ellipse(15, 280, 10, 10);
				fill(0);
				text("--> Intermediate", 30, 285);
				
				fill(color(255, 0, 0));
				ellipse(15, 310, 10, 10);
				fill(0);
				text("--> Deep", 30, 315);
	...
	private void printQuakes() 
	{
		...
		int oceanQuakes = countryMarkers.size();
		
		for (Marker cm : countryMarkers) {
			int numQuakes = 0;
			String name = cm.getProperty("name").toString();
			
			for (Marker em : quakeMarkers) {
				if(name.equals(em.getProperty("country"))) {
					numQuakes++;
				}
			}
			
			System.out.println(name + " : " + numQuakes);
			oceanQuakes -= numQuakes;
		}
